...
namespace Vanara.Windows.Forms
{
	...
	public partial class IPAddressBox : Control
	{
		...
		public IPAddress IPAddress
		{
			...
			{
				var ip = UIntPtr.Zero;
				SendMessage(ComCtl32.IPAddressMessage.IPM_GETADDRESS, UIntPtr.Zero, ip);
				return new IPAddress(ComCtl32.GET_IPADDRESS(ip));
			}
			set
			{
				...
				try { SendMessage(ComCtl32.IPAddressMessage.IPM_SETADDRESS, UIntPtr.Zero, ComCtl32.MAKEIPADDRESS(value?.GetAddressBytes())); }
		...
		public bool IsBlank => SendMessage(ComCtl32.IPAddressMessage.IPM_ISBLANK).ToInt32() > 0;
		...
		protected override CreateParams CreateParams
		{
			get
			{
				...
				cp.ClassName = ComCtl32.WC_IPADDRESS;
				cp.ExStyle &= (~(int)User32.WindowStylesEx.WS_EX_CLIENTEDGE);
		...
		public void Clear()
		{
			SendMessage(ComCtl32.IPAddressMessage.IPM_CLEARADDRESS);
		}
		...
		public bool SetFieldRange(int field, byte minValue, byte maxValue)
		{
			...
			return SendMessage(ComCtl32.IPAddressMessage.IPM_SETRANGE, (UIntPtr)field, ComCtl32.MAKEIPRANGE(minValue, maxValue)).ToInt32() > 0;
		}
		...
		{
			if (m.Msg == (int)User32.WindowMessage.WM_REFLECT + (int)User32.WindowMessage.WM_NOTIFY)
				if (WmReflectNotify(ref m))
					return;
			base.WndProc(ref m);
		}

		private IntPtr SendMessage(ComCtl32.IPAddressMessage msg, UIntPtr wParam = default(UIntPtr), UIntPtr lParam = default(UIntPtr)) =>
			User32.SendMessage(new HandleRef(this, Handle), (uint)msg, wParam, lParam);
	}

...
namespace Vanara.Extensions
{
	...
	public static partial class ExtensionMethods
	{
		...
		public static void AddRoundedRectPath(this GraphicsPath gp, Rectangle rect, Size arcSize, Corners corners = Corners.All)
		{
			if (arcSize == Size.Empty)
				gp.AddRectangle(rect);
			else
			{
				EnumFlagIndexer<Corners> crn = corners;
				var r = new Rectangle(rect.Location, arcSize);
				if (crn[Corners.TopLeft])
					gp.AddArc(r, 180, 90);
				else
					gp.AddLine(r.Location, r.Location);
				r.X = rect.Right - arcSize.Width;
				if (crn[Corners.TopRight])
					gp.AddArc(r, 270, 90);
				else
					gp.AddLine(r.X + r.Width, r.Y, r.X + r.Width, r.Y);
				r.Y = rect.Bottom - arcSize.Height;
				if (crn[Corners.BottomRight])
					gp.AddArc(r, 0, 90);
				else
					gp.AddLine(r.X + r.Width, r.Y + r.Height, r.X + r.Width, r.Y + r.Height);
				r.X = rect.Left;
				if (crn[Corners.BottomLeft])
					gp.AddArc(r, 90, 90);
				else

...
namespace Vanara.Windows.Forms
{
	...
	public abstract class CustomDrawBase : Control, IButtonControl, INotifyPropertyChanged
	{
		...
		public Image Image
		{
			...
			set { if (image.Image != value) image.Image = value; else OnPropertyChanged(nameof(Image)); }
		}
		...
		{
			get { return image.ImageIndex; }
			set { if (image.ImageIndex != value) image.ImageIndex = value; else OnPropertyChanged(nameof(ImageIndex)); }
		}
		...
		{
			get { return image.ImageKey; }
			set { if (image.ImageKey != value) image.ImageKey = value; else OnPropertyChanged(nameof(ImageKey)); }
		}
		...
		{
			get { return image.ImageList; }
			set { if (image.ImageList != value) image.ImageList = value; else OnPropertyChanged(nameof(ImageList)); }
		}
		...
		{
			get { return state[ControlState.Animating]; }
		...
		{
			get { return state[ControlState.Defaulted]; }
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
			if (e.Button == MouseButtons.Left && state[ControlState.Pressed])
			{
				...
				if (SetState(ControlState.MouseDown | ControlState.Pressed, false, false))
					Refresh();
				if (mouseWasDown && User32.WindowFromPoint(PointToScreen(e.Location)) == Handle)
				{
					OnClick(e);
					OnMouseClick(e);
				}
		...
		protected override void WndProc(ref Message m)
		{
			switch (m.Msg)
			{
				...
				case (int)ComCtl32.ButtonMessage.BM_CLICK:
					PerformClick();
					return;

				case (int)ComCtl32.ButtonMessage.BM_SETSTATE:
				...
				case (int)User32.WindowMessage.WM_CANCELMODE:

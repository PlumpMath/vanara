...
namespace Vanara.Windows.Shell
{
	...
	public sealed class ShellLink : ShellFileInfo, IDisposable
	{
		...
		{
			get { return GetStringValue(link.GetArguments, 260); }
		...
		{
			get { return GetStringValue(link.GetDescription, 1024); }
		...
		{
			get { return GetPath(SLGP.SLGP_RELATIVEPRIORITY); }
		...
		{
			get { return (FormWindowState)link.GetShowCmd() - 1; }
			set { link.SetShowCmd((uint)value + 1); Save(); }
		}

		/// <summary>Gets/sets the fully qualified path to the link's target</summary>
		public string TargetPath
		{
			get { return GetPath(SLGP.SLGP_UNCPRIORITY); }
		...
		{
			get { return GetStringValue(link.GetWorkingDirectory, 260); }
		...
		private Icon GetIcon(bool large)
		{
			...
			var iconIndex = 0;
			var iconPath = new StringBuilder(Kernel32.MAX_PATH, Kernel32.MAX_PATH);
		...
		{
			var target = new StringBuilder(Kernel32.MAX_PATH, Kernel32.MAX_PATH);

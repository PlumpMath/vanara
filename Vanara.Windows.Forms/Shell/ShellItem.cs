...
namespace Vanara.Windows.Shell
{
	...
	public class ShellItem : IComparable<ShellItem>, IDisposable, IEquatable<IShellItem>, IEquatable<ShellItem>
	{
		...
		public static ShellItem Desktop => desktop ?? (desktop = new ShellItem(KNOWNFOLDERID_Enum.FOLDERID_Desktop));
		...
		public Icon SystemIcon
		{
			get
			{
				...
				var hIcon = ComCtl32.ImageList_GetIcon(hImageList, shfi.iIcon, ComCtl32.IMAGELISTDRAWFLAGS.ILD_NORMAL);
		...
		{
			return iShellItem.Compare(other, SICHINT.SICHINT_ALLFIELDS) == 0;
		}
		...
		public IShellFolder GetIShellFolder()
		{
			try
			{
				return (IShellFolder)iShellItem.BindToHandler(null, BHID.BHID_SFObject, typeof(IShellFolder).GUID);
			}
			catch { }
			return null;
		}
		...
		public string GetToolTip(GetInfoTipFlags flags = GetInfoTipFlags.QITIPF_DEFAULT)
		{
			try
			{
		...
		int IComparable<ShellItem>.CompareTo(ShellItem other) => iShellItem.Compare(other.iShellItem, SICHINT.SICHINT_ALLFIELDS);
		...
		private class ShellItemImpl : IDisposable, IShellItem
		{
			...
			{
				PIDL dtPidl;
				SHGetFolderLocation(IntPtr.Zero, 0, AdvApi32.SafeTokenHandle.Null, 0, out dtPidl);

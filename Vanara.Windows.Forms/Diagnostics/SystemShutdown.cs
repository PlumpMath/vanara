...
namespace Vanara.Diagnostics
{
	public static class SystemShutdown
	{
		...
		public static bool ExitWindows(User32.ExitWindowsFlags flags = User32.ExitWindowsFlags.EWX_LOGOFF, AdvApi32.SystemShutDownReason reason = AdvApi32.SystemShutDownReason.SHTDN_REASON_UNKNOWN)
		{
			var reqPriv = (flags & (User32.ExitWindowsFlags.EWX_POWEROFF | User32.ExitWindowsFlags.EWX_REBOOT | User32.ExitWindowsFlags.EWX_SHUTDOWN)) != 0;
		...
		public static void InitiateShutdown(string machineName, string message, TimeSpan gracePeriod, AdvApi32.ShutdownFlags flags, AdvApi32.SystemShutDownReason reason = AdvApi32.SystemShutDownReason.SHTDN_REASON_UNKNOWN)
		{
			var graceSecs = (uint)gracePeriod.TotalSeconds;
			if (graceSecs > AdvApi32.MAX_SHUTDOWN_TIMEOUT) throw new ArgumentOutOfRangeException(nameof(gracePeriod), $"Grace period can be no longer than {AdvApi32.MAX_SHUTDOWN_TIMEOUT} seconds.");
		...
		public static bool InitiateShutdown(string machineName = null, string message = null, TimeSpan? timeout = null, bool forceAppsClosed = false, bool rebootAfterShutdown = false, AdvApi32.SystemShutDownReason reason = AdvApi32.SystemShutDownReason.SHTDN_REASON_UNKNOWN)
		{
			...
			if (Environment.OSVersion.Version.Major <= 5 && graceSecs > AdvApi32.MAX_SHUTDOWN_TIMEOUT) throw new ArgumentOutOfRangeException(nameof(timeout), $"Timeout can be no longer than {AdvApi32.MAX_SHUTDOWN_TIMEOUT} seconds on Windows XP and earlier systems.");
			using (string.IsNullOrEmpty(machineName) ? new PrivilegedCodeBlock(SystemPrivilege.Shutdown) : new PrivilegedCodeBlock(SystemPrivilege.RemoteShutdown))
		...
		{
			string reason;
			User32.ShutdownBlockReasonQuery(new HandleRef(form, form.Handle), out reason);
			return reason;
		}
		...
		public static bool ShutdownBlockReasonRemove(this Form form) => User32.ShutdownBlockReasonDestroy(new HandleRef(form, form.Handle));
		...
		/// <returns>If the call succeeds, the return value is nonzero. If the call fails, the return value is zero. To get extended error information, call GetLastError.</returns>
		public static bool ShutdownBlockReasonSet(this Form form, string reason) => User32.ShutdownBlockReasonCreate(new HandleRef(form, form.Handle), reason);
	}
}

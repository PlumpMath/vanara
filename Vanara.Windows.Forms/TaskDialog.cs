...
namespace Vanara.Windows.Forms
{
	...
	internal enum TaskDialogNotification
	{
		...
		/// <summary>
		/// Sent by the Task Dialog when the user selects a button or command link in the task dialog. The button ID corresponding to the button selected will be
		/// available in the TaskDialogNotificationArgs. To prevent the Task Dialog from closing, the application must return true, otherwise the Task Dialog
		/// will be closed and the button ID returned to via the original application call.
		/// </summary>
		ButtonClicked = 2, // wParam = Button ID

		/// <summary>
		/// Sent by the Task Dialog when the user clicks on a hyper-link in the Task Dialog’s content. The string containing the HREF of the hyper-link will be
		/// available in the TaskDialogNotificationArgs. To prevent the TaskDialog from shell executing the hyper-link, the application must return TRUE,
		/// otherwise ShellExecute will be called.
		/// </summary>
		HyperlinkClicked = 3, // lParam = (LPCWSTR)pszHREF

		/// <summary>
		/// Sent by the Task Dialog approximately every 200 milliseconds when TaskDialog.CallbackTimer has been set to true. The number of milliseconds since the
		/// dialog was created or the notification returned true is available on the TaskDialogNotificationArgs. To reset the tick count, the application must
		/// return true, otherwise the tick count will continue to increment.
		/// </summary>
		...
		/// <summary>
		/// Sent by the Task Dialog when the user selects a radio button in the task dialog. The button ID corresponding to the button selected will be available
		/// in the TaskDialogNotificationArgs. The value returned by the callback is ignored.
		/// </summary>
		...
		/// <summary>Sent by the Task Dialog when the user presses F1 on the keyboard while the dialog has focus. The value returned by the callback is ignored.</summary>
		Help = 9,

		/// <summary>
		/// Sent by the task dialog when the user clicks on the dialog's expando button. The expanded value is available on the TaskDialogNotificationArgs. The
		/// value returned by the callback is ignored.
		/// </summary>
	...
	public class TaskDialog : CommonDialog, IWin32Window
	{
		...
		private EnumFlagIndexer<TaskDialogFlags> flags;
		...
		{
			get { return flags[TaskDialogFlags.TDF_ALLOW_DIALOG_CANCELLATION]; }
			set { flags[TaskDialogFlags.TDF_ALLOW_DIALOG_CANCELLATION] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_CALLBACK_TIMER]; }
			set { flags[TaskDialogFlags.TDF_CALLBACK_TIMER] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_CAN_BE_MINIMIZED]; }
			set { flags[TaskDialogFlags.TDF_CAN_BE_MINIMIZED] = value; }
		}
		...
		public string Content
		{
			...
			set
			{
				...
				if (handle != IntPtr.Zero)
					SendMessage(HRef, (uint)TaskDialogMessage.TDM_SET_ELEMENT_TEXT, (int)TaskDialogElement.TDE_CONTENT, content);
			}
		}
		...
		public Icon CustomFooterIcon
		{
			...
			set
			{
				...
				if (handle != IntPtr.Zero)
					SendMessage(HRef, (uint)TaskDialogMessage.TDM_UPDATE_ICON, (IntPtr)TaskDialogIconElement.TDIE_ICON_FOOTER,
		...
		public Icon CustomMainIcon
		{
			...
			set
			{
				...
				if (handle != IntPtr.Zero)
					SendMessage(HRef, (uint)TaskDialogMessage.TDM_UPDATE_ICON, (IntPtr)TaskDialogIconElement.TDIE_ICON_MAIN,
		...
		{
			get { return flags[TaskDialogFlags.TDF_ENABLE_HYPERLINKS]; }
			set { flags[TaskDialogFlags.TDF_ENABLE_HYPERLINKS] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_EXPANDED_BY_DEFAULT]; }
			set { flags[TaskDialogFlags.TDF_EXPANDED_BY_DEFAULT] = value; }
		}
			...
			set
			{
				...
				if (handle != IntPtr.Zero)
					SendMessage(HRef, (uint)TaskDialogMessage.TDM_SET_ELEMENT_TEXT,
						(IntPtr)TaskDialogElement.TDE_EXPANDED_INFORMATION, expandedInformation);
			}
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_EXPAND_FOOTER_AREA]; }
			set { flags[TaskDialogFlags.TDF_EXPAND_FOOTER_AREA] = value; }
		}
		...
		public string Footer
		{
			...
			set
			{
				...
				footer = value;
				if (handle != IntPtr.Zero)
					SendMessage(HRef, (uint)TaskDialogMessage.TDM_SET_ELEMENT_TEXT, (IntPtr)TaskDialogElement.TDE_FOOTER, footer);
			}
		}
		...
		public string MainInstruction
		{
			...
			set
			{
				if (mainInstruction != value)
				{
					...
					if (handle != IntPtr.Zero)
						SendMessage(HRef, (uint)TaskDialogMessage.TDM_SET_ELEMENT_TEXT, (IntPtr)TaskDialogElement.TDE_MAIN_INSTRUCTION,
							mainInstruction);
				}
			}
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_NO_DEFAULT_RADIO_BUTTON]; }
			set { flags[TaskDialogFlags.TDF_NO_DEFAULT_RADIO_BUTTON] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_POSITION_RELATIVE_TO_WINDOW]; }
			set { flags[TaskDialogFlags.TDF_POSITION_RELATIVE_TO_WINDOW] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_RTL_LAYOUT]; }
			set { flags[TaskDialogFlags.TDF_RTL_LAYOUT] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_SIZE_TO_CONTENT]; }
			set { flags[TaskDialogFlags.TDF_SIZE_TO_CONTENT] = value; }
		}
		...
		public bool TopMost
		{
			get { return !flags[TaskDialogFlags.TDF_NO_SET_FOREGROUND]; }
			set { flags[TaskDialogFlags.TDF_NO_SET_FOREGROUND] = !value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_VERIFICATION_FLAG_CHECKED]; }
			set { flags[TaskDialogFlags.TDF_VERIFICATION_FLAG_CHECKED] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_SHOW_MARQUEE_PROGRESS_BAR]; }
			set { flags[TaskDialogFlags.TDF_SHOW_MARQUEE_PROGRESS_BAR] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_SHOW_PROGRESS_BAR]; }
			set { flags[TaskDialogFlags.TDF_SHOW_PROGRESS_BAR] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_USE_COMMAND_LINKS]; }
			set { flags[TaskDialogFlags.TDF_USE_COMMAND_LINKS] = value; }
		}
		...
		{
			get { return flags[TaskDialogFlags.TDF_USE_COMMAND_LINKS_NO_ICON]; }
			set { flags[TaskDialogFlags.TDF_USE_COMMAND_LINKS_NO_ICON] = value; }
		}
		...
		private int PrivateCallback([In] IntPtr hwnd, [In] int msg, [In] IntPtr wparam, [In] IntPtr lparam,
			[In] IntPtr refData) => HookProc(hwnd, msg, wparam, lparam).ToInt32();
		...
		private TaskDialogResult PrivateShow(IntPtr hwndOwner)
		{
			...
			var config = new TASKDIALOGCONFIG
			{
				...
				dwFlags = flags,
				dwCommonButtons = (TaskDialogCommonButtonFlags)CommonButtons
			};
			...
			{
				config.dwFlags |= TaskDialogFlags.TDF_USE_HICON_MAIN;
			...
			{
				config.dwFlags |= TaskDialogFlags.TDF_USE_HICON_FOOTER;
		...
		private struct EnumFlagIndexer<TE> where TE : struct, IComparable
		{
			private TE flags;
			...
			public static TE operator &(EnumFlagIndexer<TE> a, TE b)
				=> (TE)Enum.ToObject(typeof(TE), Convert.ToInt64(a.flags) & Convert.ToInt64(b));

			public static TE operator |(EnumFlagIndexer<TE> a, TE b)
				=> (TE)Enum.ToObject(typeof(TE), Convert.ToInt64(a.flags) | Convert.ToInt64(b));

			public static bool operator ==(EnumFlagIndexer<TE> a, TE b) => Convert.ToInt64(a.flags) == Convert.ToInt64(b);
			public void Clear() { flags = default(TE); }
			...
			public void Union(TE otherFlags) { this[otherFlags] = true; }

			public void Union(IEnumerable<TE> otherFlags)
			{
				foreach (var e in otherFlags) this[e] = true;
			}
		}
		...
		public abstract class TaskDialogButtonBase : IEquatable<TaskDialogButtonBase>
		{
			...
			public int ButtonId
			{
				get { return nativeButton.buttonId; }
				set { nativeButton.buttonId = value; }
			}
			...
			{
				get { return nativeButton.buttonText; }
				set { nativeButton.buttonText = value; }
			}
		...
		public class TaskDialogButtonCollection<T> : List<T>, IDisposable where T : TaskDialogButtonBase
		{
			...
			/// <summary>Gets then number of elements contained in the collection.</summary>
				...
				if (h != c.ptrHash)
				{
					...
					if (c.Count > 0)
					{
						...
						for (var i = 0; i < c.ptrCount; i++)
							Marshal.StructureToPtr(c[i].nativeButton, new IntPtr(c.ptr.ToInt64() + (i*elementSize)), false);
					}
			...
			void IDisposable.Dispose()
			{
				...
				lock (this)
				{
					...
					for (var i = 0; i < ptrCount; i++)
						Marshal.DestroyStructure(new IntPtr(ptr.ToInt64() + (i*elementSize)),
							typeof(TaskDialogButtonBase.TASKDIALOG_BUTTON));
		...
		public class TaskDialogProgressBar
		{
			...
			{
				get { return mSpeed; }
				set { if (mSpeed != value) SetMarqueeSpeed(mSpeed = value); }
			}
			...
			{
				get { return state; }
				set { if (state != value) SetState(state = value); }
			}
			...
			{
				get { return val; }
				set { if (val != value) SetValue(val = value); }
			}

			[DefaultValue(false)]
			public bool Visible
			{
				get { return taskDialog != null && (taskDialog.ShowProgressBar || taskDialog.ShowMarqueeProgressBar); }
				set
				{

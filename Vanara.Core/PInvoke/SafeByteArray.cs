...
namespace Vanara.PInvoke
{
	...
	/// <seealso cref="Vanara.PInvoke.SafeHGlobalHandle"/>
	public class SafeByteArray : SafeMemoryHandle<CoTaskMemoryMethods>, IList<byte>, ICloneable, IStructuralComparable, IStructuralEquatable, IList
	{
		...
		public SafeByteArray(byte[] array) : base(Marshal.SizeOf(typeof(byte)) * array?.Length ?? 0)
		{
			if (array != null)
		...
		public byte this[int index]
		{
			get
			{
				...
				if (IsInvalid) throw new InvalidOperationException(@"Object is not valid.");
		...
		/// <param name="index">The index.</param>
		/// <returns></returns>
		object IList.this[int index] { get { return this[index]; } set { this[index] = (byte)value; } }
		...
		{
			((IList<byte>)this).Add((byte)value);
			return Count;
		}
		...
		/// </returns>
		/// <exception cref="System.ArgumentOutOfRangeException">other - Other array not of same size.</exception>
		int IStructuralComparable.CompareTo(object other, IComparer comparer)
		{
			if (other == null)
				return 1;

			var o = other as SafeByteArray;
			if (o == null || Size != o.Size)
				throw new ArgumentOutOfRangeException(nameof(other), @"Other array not of same size.");

			for (var i = 0; i < Count; i++)
			{
				int c;
				if ((c = comparer.Compare(this[i], o[i])) != 0)
					return c;
			}
			return 0;
		}
		...
		public int IndexOf(object value) => IndexOf((byte)value);
		...
		{
			((IList<byte>)this).Remove((byte)value);
		}

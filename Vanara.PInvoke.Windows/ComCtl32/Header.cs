...
namespace Vanara.PInvoke
{
	public static partial class ComCtl32
	{
		...
		/// <summary>
		/// Flags indicating which <see cref="HDITEM"/> structure members contain valid data or must be filled in. 
		/// </summary>
		[Flags]
		...
		public sealed class HDITEM : IDisposable
		{
			...
			{
				get { return hbm == IntPtr.Zero ? null : Image.FromHbitmap(hbm); }
				set { hbm = value?.GetHbitmap() ?? IntPtr.Zero; EnumExtensions.SetFlags(ref mask, HeaderItemMask.HDI_BITMAP); }
			}

			public bool Checked
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_CHECKED); }
			...
			public object Filter
			{
				...
				set
				{
					...
					{
						if (value is DateTime) value = new SYSTEMTIME((DateTime)value);
						if (value is string) { pvFilter = new HDTEXTFILTER((string)value).StructureToPtr(); type = HeaderItemFilterType.HDFT_ISSTRING; }
						else if (value is int) { pvFilter = new IntPtr((int)value); type = HeaderItemFilterType.HDFT_ISNUMBER; }
						else if (value is SYSTEMTIME) { pvFilter = ((SYSTEMTIME)value).StructureToPtr(); type = HeaderItemFilterType.HDFT_ISDATE; }
						else throw new ArgumentException("Value must be a string, integer, DateTime or SYSTEMTIME");
					}
			...
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_FIXEDWIDTH); }
			...
			public bool Focused
			{
				get { return state == HeaderItemState.HDIS_FOCUSED; }
			...
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_BITMAP_ON_RIGHT); }
			...
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_OWNERDRAW); }
			...
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_RTLREADING); }
			...
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_CHECKBOX); }
			...
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_SPLITBUTTON); }
			...
			{
				get { return fmt.IsFlagSet(HeaderItemFormat.HDF_STRING); }
			...
			public string Text
			{
				...
				set
				{
					...
					StringExtensions.SetString(ref pszText, ref cchTextMax, value);
					EnumExtensions.SetFlags(ref mask, HeaderItemMask.HDI_TEXT);
				}
			}

			public HeaderItemFormat TextAlignment
			{
				get { return fmt & HeaderItemFormat.HDF_JUSTIFYMASK; }
				set
				{
		...
		public sealed class HDLAYOUT : IDisposable
		{
			...
			{
				prc = rc.StructureToPtr();
				pwpos = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(User32.WINDOWPOS)));
			}
			...
			{
				get { return prc.ToStructure<RECT>(); }
				set
				{
					Marshal.FreeHGlobal(prc);
					prc = value.StructureToPtr();
				}
			}

			/// <summary>Gets the position.</summary>
			/// <value>The position.</value>
			public User32.WINDOWPOS Position => pwpos.ToStructure<User32.WINDOWPOS>();
		...
		public struct HDTEXTFILTER
		{
			...
			public override string ToString() { return pszText; }
		}
		...
		public sealed class NMHEADER
		{
			...
			public int iItem;
			/// <summary>A value specifying the index of the mouse button used to generate the notification message. This member can be one of the following values:
			/// <list type="table">
			/// <listheader><term>Value</term><term>Meaning</term></listheader>
			/// <item><term>0</term><description>Left button</description></item>
			/// <item><term>1</term><description>Right button</description></item>
			/// <item><term>2</term><description>Middle button</description></item>
			/// </list>
			/// </summary>
			public int iButton;
			/// <summary>
	...
	public static partial class User32
	{
		...
		public static extern IntPtr SendMessage(HandleRef hWnd, ComCtl32.HeaderMessage message, [MarshalAs(UnmanagedType.SysInt)] int wParam, [In, Out] ComCtl32.HDITEM item);

		[DllImport(nameof(User32), SetLastError = true)]
		public static extern IntPtr SendMessage(HandleRef hWnd, ComCtl32.HeaderMessage message, [MarshalAs(UnmanagedType.SysInt)] int wParam, [In, Out] ComCtl32.HDLAYOUT layout);

		[DllImport(nameof(User32), SetLastError = true)]
		public static extern IntPtr SendMessage(HandleRef hWnd, ComCtl32.HeaderMessage message, [MarshalAs(UnmanagedType.SysInt)] int wParam, [In, Out] ComCtl32.HDHITTESTINFO hittest);
	}
}
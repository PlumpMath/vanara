...
namespace Vanara.PInvoke
{
	public static partial class AclUI
	{
		public enum SECURITY_OBJECT_ID
		{
			...
			/// <para>If Id is set to this value, then pData points to the ISecurityInformation interface of an object that represents the security context of the share.</para>
			/// <para>If the security descriptor is not yet available, then pData2 must be a handle to a waitable object that is signaled when the security descriptor is ready when the GetSecondarySecurity method returns S_FALSE. The waitable object should be created by the CreateEvent function. In this case, cbData2 is 0.</para>
			...
			/// <para>If Id is set to this value, then pData points to the security descriptor with an empty DACL, an owner, group, and attribute access control entries (ACEs) that match the resource's owner, group, and attributes as well as a SCOPE_SECURITY_INFORMATION_ACE that contains the central policy's ID. cbData is set to the number of bytes in pData.</para>
			/// <para>pData2 is NULL and cbData2 is 0.</para>
			/// <para>The security descriptor is constructed to allow computing effective permissions to correctly determine when access is limited by the central policy and higher detail of the central access rule cannot be determined. This is used when a central access policy that applies to a resource cannot be resolved into its elemental central access rules.</para>
			/// </summary>
			SECURITY_OBJECT_ID_CENTRAL_POLICY = 3,
			/// <summary>
			/// The security descriptor of a central access rule.
			/// <para>If Id is set to this value, then pData points to the security descriptor with an owner, group, and attribute ACEs that match the resource's owner, group, and attributes, and a discretionary access control list (DACL) that matches the central access rule's DACL. cbData is set to the number of bytes in pData.</para>
			/// <para>In addition, pData2 points to a security descriptor with a DACL that contains a conditional ACE that grants 0x1 to Everyone if the resource condition from the central access rule evaluates to TRUE. cbData2 is set to the number of bytes in pData2.</para>
			/// <para>The security descriptor is constructed to allow computing effective permissions to determine when access is limited by the central access policy at the highest detail. That is, access is limited by pointing to a central policy rule.</para>
			/// </summary>
		...
		/// <summary>
		/// Indicate the types of ACEs that can be inherited in a <see cref="SI_INHERIT_TYPE"/> structure.
		/// </summary>
		[Flags]
		...
		/// <summary>
		/// Combines the <see cref="SI_PAGE_TYPE"/> and <see cref="SI_PAGE_ACTIVATED"/> types for use in the last parameter of <see cref="EditSecurityAdvanced(System.IntPtr,ISecurityInformation,uint)"/> method.
		/// </summary>
		...
		[DllImport(nameof(AclUI), ExactSpelling = true, SetLastError = true)]
		[return: MarshalAs(UnmanagedType.Bool)]
		...
		[DllImport(nameof(AclUI), ExactSpelling = true, PreserveSig = true)]
		...
		public static void EditSecurityAdvanced(IntPtr hwnd, ISecurityInformation psi, SI_PAGE_TYPE pageType, SI_PAGE_ACTIVATED pageActivated)
		{
			EditSecurityAdvanced(hwnd, psi, COMBINE_PAGE_ACTIVATION(pageType, pageActivated));
		}
		...
		/// <summary>
		/// Class contains information about an access right or default access mask for a securable object.
		/// The <see cref="IAccessControlEditorDialogProvider.GetAccessListInfo"/> method uses this class to specify information that the access control editor uses to initialize its property pages.
		/// </summary>
		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]
		public sealed class SI_ACCESS : IDisposable
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="SI_ACCESS"/> class.
			/// </summary>
			...
			{
				get { return pguid == IntPtr.Zero ? Guid.Empty : (Guid)Marshal.PtrToStructure(pguid, typeof(Guid)); }
				set
				{
			...
			{
				if (pguid != EmptyGuidPtr && pguid != IntPtr.Zero)
		...
		public sealed class SI_INHERIT_TYPE : IDisposable
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="SI_INHERIT_TYPE"/> struct.
			/// </summary>
			...
			/// <param name="flags">The inheritance flags.</param>
			/// <param name="name">The display name.</param>
			public SI_INHERIT_TYPE(Guid childObjectType, SI_INHERIT_Flags flags, string name)
				: this(flags, name)
			{
		...
		public struct SI_OBJECT_INFO
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="SI_OBJECT_INFO"/> struct.
			/// </summary>
